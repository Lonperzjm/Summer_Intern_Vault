...
>
>    **✅ Guidance timing 的直接生成实证（[[wiki/sources/kynkaanniemiApplyingGuidanceLimited2024|Guidance Interval]]，2026-08-11 ingest）**：全程固定 CFG 并非最优——高噪声阶段有害、中间阶段有益、低噪声阶段基本无必要；只在中间 noise interval 引导可将 ImageNet-512 FID 从 1.81 降至 1.40，并减少区间外 unconditional forward。它把上面的旋钮从“逐 $t$ 的强度”进一步拆为 **magnitude × temporal support**，直接支持“介入时间步本身是研究杠杆”。但该结果是生成任务证据，不自动等价于 editing 的 fidelity↔controllability 结论；最优窗口也仍依赖模型、sampler 与时间参数化。
>
>    **✅ Guidance 的 trajectory-geometry 解释（[[wiki/sources/chidambaramWhatDoesGuidance2024|What Does Guidance Do?]]，2026-08-14 ingest）**：tilting 与 noising 不交换；guidance 改变 reverse dynamics，并在简单分布上把样本推向目标类边界或尾部。过强 guidance 还会进入 score error 控制较弱的低密度区。这把旋钮从 scale × timing 扩展为**对访问状态分布的控制**。简单分布的 overshoot–return 不证明真实高维 FM 存在可信恢复区，因此不改变 working thesis 版本。
>
>    **✅ Transport / steering 分离（[[wiki/methods/cfg-plus-plus|CFG++]]，2026-08-14 ingest）**：vanilla CFG 用同一外推方向做 denoise 与 renoise；CFG++ 只用 guidance 形成 clean estimate，把 transport 交回 unconditional model。它支持“guidance 不必修改整个生成 vector field”。但论文无 Flow Matching / Rectified Flow 的直接公式或验证，因此 working thesis 版本不变。
>
>    **✅ Guidance policy 学习（[[wiki/methods/learn-to-guide|Learn to Guide]]，2026-08-14 ingest）**：冻结 backbone，以 self-consistency 学习 $\omega(c,s,t)$，并在 1.05B Flow Matching 模型上实证。它扩展为 scale × timing × condition × target step，但不读取 $x_t$，故不是 event-triggered / state-adaptive；working thesis 版本不变。
>
...
>
> **2026-08-04 solver taxonomy 校准**：[[wiki/sources/flow-matching-solver-method-taxonomy|用户整理的分类笔记]]表明“采样加速”内部还需拆成积分公式、时间网格、模型专用参数、辅助网络、轨迹选择与系统并行等正交层。步数、NFE、理论阶数和墙钟不是同一指标。此项不改变 working thesis，但要求后续实验固定基础 solver、逐层消融并完整核算隐藏成本。
>
...
当前实验主线已转为 [[wiki/synthesis/reject-and-skip-research-direction|Reject-and-Skip：推理时 Coupling 干预]]。其核心不是发明一个更准确逼近 marginal ODE 的传统 solver，而是研究：条件速度平均造成局部不可信场时，能否用非 oracle detector 触发 rollback 和远端出口搜索，以严格等 NFE 的结构化离散偏置改善生成分布。

**2026-08-14 prior-art 边界（[[wiki/methods/diffusion-tree-sampling|DTS]]）**：stochastic diffusion 上的轨迹树搜索、terminal reward backup 与跨 rollout 复用已被 DTS 系统实现。Reject-and-Skip 不能再以“branch/search/reuse”作宽泛新意；当前可守边界是 deterministic FM、严格低 NFE、单轨迹 state-triggered 的局部 rollback/skip、且不依赖 terminal reward。DTS 没有 FM/ODE 实验，因此本条收窄 claim，但不改变 working thesis 版本。

二维解析 toy 已支持该机制能改变 coupling 并减少粗 Euler 异常终点；官方 CIFAR-10 1-RF 已建立真实模型数值基线和 detector 诊断，但尚未证明内部 conditional ambiguity 或真实分布收益。当前 working thesis 已同步到 [[research/thesis]] v0.2；方法定义见 [[wiki/concepts/reject-and-skip]]。
...
- 推理时状态可信度检测、velocity return 与 transport coupling 干预
...
- **guidance 旋钮**：[[wiki/concepts/classifier-free-guidance|Classifier-Free Guidance]]（✅ [[wiki/sources/dhariwalDiffusionModelsBeat2021|Classifier Guidance]]；✅ [[wiki/sources/hoClassifierFreeDiffusionGuidance2022|CFG]]；✅ [[wiki/sources/kynkaanniemiApplyingGuidanceLimited2024|Guidance Interval]]；✅ [[wiki/sources/chidambaramWhatDoesGuidance2024|What Does Guidance Do?]]；✅ [[wiki/methods/cfg-plus-plus|CFG++]]；✅ [[wiki/methods/learn-to-guide|Learn to Guide]]）——脉络扩展到 timing、visited-state distribution、denoise/renoise decomposition 与 condition-/step-dependent policy。Learn to Guide 已给出 Flow Matching 实证；仍开放 state-aware guidance 及其与 operator splitting 的组合。
- **inference-time search / scaling**：[[wiki/methods/diffusion-tree-sampling|DTS / DTS$^\star$]] 把 stochastic denoising 变为持久树，用 terminal reward backup 复用跨 rollout 计算；DTS 做 posterior sampling，DTS$^\star$ 做 high-reward search。它是 Reject-and-Skip 的直接搜索型 prior art，但 deterministic FM 的低预算局部树/skip 仍未解决。
- **采样加速线**：[[wiki/sources/songDenoisingDiffusionImplicit2022|DDIM]]、[[wiki/sources/songScoreBasedGenerativeModeling2021|Score SDE]]、[[wiki/sources/lipmanFlowMatchingGenerative2023|Flow Matching]] 与 [[wiki/sources/liuFlowStraightFast2022a|Rectified Flow]] 已 ingest（验证推论 3——DDIM 给离散跳步，Score SDE 给 [[wiki/concepts/probability-flow-ode|PF-ODE]] 连续总纲 + PC 校正，FM 给 [[wiki/concepts/optimal-transport-path|OT 路径]]级的 NFE 下降，RF 给 [[wiki/concepts/reflow|reflow]] 训练阶段直线化 + 1-step 蒸馏入口）；🆕 **inference-time spatial refinement**：[[wiki/sources/chaTrainingFreeRefinementFlow2026|FDS (Cha et al. 2026)]] 已 ingest——不改训练，用 velocity divergence 做 discrepancy proxy 在采样时把轨迹推离高歧义区域，与 temporal refinement（高阶 solver）正交，plug-and-play 叠加任何 solver/guidance，在 CIFAR-10/ImageNet 直接超过 HRF 等 training-based 方法；🆕 **learnt solver optimization**：[[wiki/sources/shaulBespokeSolversGenerative2023|Bespoke Solvers (Shaul et al. 2023)]] 已 ingest——为特定预训练 flow model 学习最优 scale-time 变换（~80 参数），保持 solver 一致性（阶数不变）的同时显著降低低 NFE 下的全局截断误差；Theorem 2.3 证明搜索 scale-time 变换等价于搜索所有 Gaussian Path（统一所有 noise schedule），是全局离线优化路线，与 FDS 的在线 spatial refinement 和我们的在线 temporal adaptive 策略正交；**对 thesis 高度相关**——可与 energy-guidance 组合（divergence = "where to refine", energy = "how to refine"）。🆕 **learnt velocity interpolation**：[[wiki/sources/chenBiAnchorInterpolationSolver2026|BA-solver (Chen et al. 2026, ICML)]] 已 ingest——冻结 backbone + 轻量 SideNet（~1% 参数）在双 anchor 之间做区间内速度插值 + 高阶求积，7 NFE FID 1.96（ImageNet-256），训练代价极低（250 iter / 6M params）；属 ode-solver-taxonomy 中 solver distillation 与模型蒸馏之间的新档；不处理速度平均化问题（与 FDS/thesis 正交）。下一步：Consistency Models（Yang Song 后续）、SD3 / FLUX（RF 工业实现，填 overview「主要派系→flow-matching-based」）、RF-Inversion 系（RF backbone 上的编辑方法，直接对接 thesis）—— 这些对**编辑质量**的影响仍是空白
...
- **DDIM inversion**：DDIM 的 ODE 反向积分是 inversion-based 编辑底座。✅ [[wiki/methods/cfg-plus-plus|CFG++]] 提供 guidance-aware 修正：vanilla CFG 放大跨步 mismatch，CFG++ 用 guided denoising + unconditional renoising 降低该误差。仍需区分 DDIM 离散误差、guidance-induced mismatch 与 LDM autoencoder 重建上限。
