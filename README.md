...
2. 工作记录(简要版，详见由ai编写的[[log]])：
	...
	* 6.1-6.7 完成 **DBIM**、**Stochastic Interpolants**、**FLUX.1 Kontext**。完成 [[raw/literature-notes/zhengDiffusionBridgeImplicit2025|zhengDiffusionBridgeImplicit2025]]、[[raw/literature-notes/labsFLUX1KontextFlow2025|labsFLUX1KontextFlow2025]] 阅读笔记，`ingest` 三篇（SI 为 PDF cold ingest）。**选题止损**：沿 bridge-SDE 方向连做三次文献 sweep，确认三条候选 carve 均被占——"脱离 diffusion 的 SDE bridge"撞 [[wiki/sources/albergoStochasticInterpolants2023|Stochastic Interpolants]]、bridge-SDE 加速撞 CDBM、可控多样性编辑撞 OSCAR；[[wiki/sources/labsFLUX1KontextFlow2025|FLUX.1 Kontext]] 进一步表明编辑工业主线是 **in-context 条件 FM 而非 bridge**。结论：红海里不靠 blue-sky 新意，转向 FlowCycle 弱点分析 / 窄 niche / 与导师定向（详见 [[wiki/synthesis/bridge-sde-editing-landscape]]、[[research/ideas]]）。
